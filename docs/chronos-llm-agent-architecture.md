...
- 已接入 `TaskSemanticPlanningAgent`，使用 prompt registry 中的 `p2-task-semantic-planning-agent-v2`。
...
- Agent 输出落库为 `TaskPlanningSignal`，包含 `task_type`、`complexity`、`cognitive_load`、`energy_fit`、`blocking_risk`、`estimated_duration_min`、`goal_alignment_score`、`semantic_priority_score`、`minimum_viable_step`；v2 额外把 `complexity_reason`、`duration_reason`、`goal_progress_impact`、`goal_relevance_reason` 和 `minimum_viable_minutes` 保存在 `raw_payload`，作为 Planning Engine 的可解释语义输入。
...
- Planning Engine 只读取 fresh TaskPlanningSignal，并在 `DailyPlanItem.score_breakdown` 写入 `semantic_signal_applied`、`semantic_total_score`、`goal_alignment_signal_score`、`semantic_priority_signal_score`、`semantic_goal_progress_impact_score`、`semantic_goal_relevance_reason`、`semantic_minimum_viable_step`、`semantic_minimum_viable_minutes` 等分项。
