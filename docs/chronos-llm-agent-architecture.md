...
- 已提供 `scripts/evaluate_planning_engine.py` 固定场景评估，覆盖容量滚动、受保护任务超载、低精力保护和高精力深度任务适配；支持 `--jsonl-output` 写出 run summary 和 scenario records，后续 LLM Daily Planner 必须通过这些基线或显式更新评估预期。
