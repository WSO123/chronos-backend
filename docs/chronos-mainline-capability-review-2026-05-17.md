...
| Planning Engine / Today 编排 | L3 | 确定性排序是 source of truth，读取价值、deadline、Goal 完成率 / 剩余压力、剩余估时、依赖、用户修正、执行反馈、容量、Energy、fresh TaskPlanningSignal 语义信号和同类任务历史执行画像 | 后续继续补边界场景和回归基线，不让 LLM 接管排序 |
...
| Planner Review Feedback v1 | L2-L3 | 用户可接受或忽略 Strategy Detail 的 planner suggestion，系统以 ActivityEvent 记录，并压缩成确定性 `feedback_summary` 进入后续 Daily Planner 和 Strategy Explanation 只读上下文 | 不立即 replan，不改任务，不让 LLM 直接学习成隐藏排序器 |
