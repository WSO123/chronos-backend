...
class PlanningEngineEvaluationTests(unittest.TestCase):
    def test_fixed_scenarios_pass(self):
        ...
        self.assertEqual(
            {scenario["name"] for scenario in result["scenarios"]},
            {
                ...
                "execution_learning_calibrates_focus_history",
                "semantic_coverage_v2_guides_minimum_goal_action",
        ...
        for scenario in result["scenarios"]:
            ...
            for item_signal in scenario["details"]["item_signals"]:
                ...
                self.assertIn("semantic_goal_progress_impact_score", item_signal)
                self.assertIn("execution_learning_applied", item_signal)
                self.assertIn("planning_objective_execution_learning_component", item_signal)
    ...
    def test_write_jsonl_result_outputs_summary_and_scenario_records(self):
        ...
        self.assertEqual(records[16]["record_type"], "run_summary")
        self.assertEqual({record["record_type"] for record in records[1:16]}, {"scenario_result"})
        self.assertEqual(
            {record["scenario_name"] for record in records[1:16]},
            {
                "capacity_rollover",
                "capacity_objective_protects_goal_progress",
                "protected_overload_warning",
        ...
        self.assertIn("semantic_goal_impact_count", records[1]["details"])
        self.assertIn("execution_learning_signal_count", records[1]["details"])
